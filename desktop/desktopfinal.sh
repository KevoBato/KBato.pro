...
apt install -y /tmp/$CHROME_DEB
rm /tmp/$CHROME_DEB

# Step 6: Create the desktop.sh file in Ports to launch Chrome in fullscreen
echo "Creating desktop.sh to launch Chrome in fullscreen..."
mkdir -p /userdata/roms/ports
cat << 'EOF' > /userdata/roms/ports/desktop.sh
#!/bin/bash
...
EOF
chmod +x /userdata/roms/ports/desktop.sh

# Step 7: Create the Google Chrome launcher script in Ports
...
# Step 8: Create desktop entry for Chrome
APPNAME="Chrome"
...
# Step 10: Download the Chrome logo for the menu
...
xmlstarlet ed -s "/gameList" -t elem -n "game" -v "" \
  ...
  /userdata/roms/ports/gamelist.xml > /userdata/roms/
